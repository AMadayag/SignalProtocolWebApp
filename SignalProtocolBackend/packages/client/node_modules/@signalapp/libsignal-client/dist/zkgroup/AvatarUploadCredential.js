...
export class AvatarUploadCredentialRequestContext extends ByteArray {
    ...
    constructor(contents) {
        super(contents, Native.AvatarUploadCredentialRequestContext_CheckValidContents);
    }
    /**
     ...
     */
    static create(aci, zkCredentialKeyPair, rotationId) {
     ...
     */
    static createWithRandom(aci, zkCredentialKeyPair, rotationId, random) {
        const newContents = Native.AvatarUploadCredentialRequestContext_New(aci.getServiceIdFixedWidthBinary(), zkCredentialKeyPair.getContents(), rotationId, random);
        return new AvatarUploadCredentialRequestContext(newContents);
    }
    /** The request to send to the issuing server. */
    getRequest() {
        const newContents = Native.AvatarUploadCredentialRequestContext_GetRequest(this.contents);
     ...
     */
    receiveResponse(response, params, now = new Date()) {
        const newContents = Native.AvatarUploadCredentialRequestContext_ReceiveResponse(this.getContents(), response.getContents(), Math.floor(now.getTime() / 1000), params.getContents());
...
export class AvatarUploadCredentialRequest extends ByteArray {
     ...
     */
    issueCredential(aci, zkCredentialKeyPublic, rotationId, redemptionTime, params) {
        const random = randomBytes(RANDOM_LENGTH);
        return this.issueCredentialWithRandom(aci, zkCredentialKeyPublic, rotationId, redemptionTime, params, random);
    }
    /**
     ...
     * if the source of randomness doesn't matter.
     */
    issueCredentialWithRandom(aci, zkCredentialKeyPublic, rotationId, redemptionTime, params, random) {
        const newContents = Native.AvatarUploadCredentialRequest_IssueDeterministic(this.getContents(), aci.getServiceIdFixedWidthBinary(), zkCredentialKeyPublic.getContents(), rotationId, Math.floor(redemptionTime.getTime() / 1000), params.getContents(), random);
...
export class AvatarUploadCredential extends ByteArray {
    ...
    presentWithRandom(serverParams, random) {
        const newContents = Native.AvatarUploadCredential_PresentDeterministic(this.getContents(), serverParams.getContents(), random);
    ...
    getRedemptionTime() {
        return new Date(1000 * Native.AvatarUploadCredential_GetRedemptionTime(this.contents));
    }
}
...
export class AvatarUploadCredentialPresentation extends ByteArray {
    __type;
    constructor(contents) {
        super(contents, Native.AvatarUploadCredentialPresentation_CheckValidContents);
    }
    /**
    ...
    verify(serverParams, now = new Date()) {
        Native.AvatarUploadCredentialPresentation_Verify(this.getContents(), Math.floor(now.getTime() / 1000), serverParams.getContents());
    }
    /**
    ...
    getRedemptionTime() {
        return new Date(1000 *
            Native.AvatarUploadCredentialPresentation_GetRedemptionTime(this.contents));
    }
}
//# sourceMappingURL=AvatarUploadCredential.js.map